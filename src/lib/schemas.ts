...
export const EbookSchema = z
    ...
    .superRefine((data, ctx) => {
        if (data.series && data.series_index == null) {
            ctx.addIssue({
                code: "custom",
                message: "Series index is required, if series is selected.",
                path: ["series_index"],
            });
        }
        if (data.series_index != null && data.series == null) {
            ctx.addIssue({
                code: "custom",
                message: "Series is required, if series index is selected.",
                path: ["series"],
            });
        }
    });
...
function customValidate(
    ...
    return z.any().transform((val, ctx) => {
        ...
        if (!r.success) {
            ...
            if (
                r.error?.issues &&
                r.error.issues.findIndex((i) => i.code === "too_big") !== -1
            ) {
